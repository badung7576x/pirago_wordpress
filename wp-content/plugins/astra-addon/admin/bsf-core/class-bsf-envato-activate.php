...
class BSF_Envato_Activate {
	...
	public function envato_register( $args ) {
		...
		$form_class                   = ( isset( $args['form_class'] ) && ! is_null( $args['form_class'] ) ) ? $args['form_class'] : "bsf-license-form-{'10'}";
		...
		$form_action = bsf_registration_page_url( '', '10' );
		...
		$envato_active_oauth_title    = apply_filters( "envato_active_oauth_title_{'10'}", 'Updates & Support Registration - <span class="active">Active!</span>' );
	...
	protected function create_token( $product_id ) {
		$token = '10' . '|' . current_time( 'timestamp' ) . '|' . bsf_generate_rand_token();
	...
	protected function process_envato_activation() {
		...
		$product_id = '10';
			...
			$args                 = array();
			$args['purchase_key'] = 'hotrowordpress';
