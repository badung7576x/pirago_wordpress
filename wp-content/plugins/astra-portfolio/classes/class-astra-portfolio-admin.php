...
if ( ! class_exists( 'Astra_Portfolio_Admin' ) ) :
	...
	class Astra_Portfolio_Admin {
		...
		public function post_updated( $post_ID, $post, $update ) {
			...
			// Skip for portfolio update. Trigger only for new portfolios.
			if ( $update ) {
				return;
			}

			$exclude_ids    = (array) get_option( 'astra_portfolio_excludes', array() );
			$portfolio_type = get_post_meta( $post_ID, 'astra-portfolio-type', true );

			if ( $this->is_portfolio_type_excluded_from_search( $portfolio_type ) ) {
				$exclude_ids   = (array) get_option( 'astra_portfolio_excludes', array() );
				$exclude_ids[] = $post_ID;
		 ...
		 */
		private static function get_portfolio_image_url( $image_id = '' ) {
		...
		public function meta_boxe_callback( $post ) {
			...
			$thumbnail_url = self::get_portfolio_image_url( $thumbnail_id );

			$portfolio_featured_id  = get_post_meta( $post->ID, 'astra-portfolio-image-id', true );
			$portfolio_featured_url = self::get_portfolio_image_url( $portfolio_featured_id );
			...
			<table class="widefat astra-portfolio-table">
				<tr class="astra-portfolio-row">
					...
					<td class="astra-portfolio-content">
						<?php
						echo esc_html( self::get_portfolio_type_label( $portfolio_type ) );
						?>
					</td>
				</tr>
				<tr class="astra-portfolio-row">
					...
					<td class="astra-portfolio-content">
						<div class="astra-portfolio-image">
							<?php self::image_markup( $portfolio_featured_url ); ?>
					...
					case 'image':
						?>
									<tr class="astra-portfolio-row">
										...
										<td class="astra-portfolio-content">
											<div class="astra-portfolio-image">
												<?php self::image_markup( $thumbnail_url ); ?>
		 ...
		 * @return void
		 */
		public static function add_terms( $taxonomy = '', $terms = array() ) {
		...
		public function where_clause( $where, $query ) {
			...
			// return if exclude.
			$exclude_ids = self::get_excluded_items();
		...
		public function hidden_post_string() {
			return implode( ', ', self::get_excluded_items() );
		}
